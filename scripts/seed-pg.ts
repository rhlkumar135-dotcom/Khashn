...
const communities = [
  ...
  { slug: 'motor-city', nameEn: 'Motor City', nameAr: 'موتور سيتي', emirate: 'dubai', lat: 25.0460, lng: 55.2450, median: 1150, rent: 62000, yield: 6.3, score: 72, s30d: 1.0, s1y: 10.8, t30: 134, total: 13400, schools: 6, health: 5, metro: 2, retail: 6, parks: 7, worship: 6 },
  ...
  { slug: 'al-reem-island', nameEn: 'Al Reem Island', nameAr: 'جزيرة ريم', emirate: 'abu_dhabi', lat: 24.4880, lng: 54.3530, median: 1600, rent: 88000, yield: 6.4, score: 80, s30d: 1.9, s1y: 16.8, t30: 234, total: 19800, schools: 7, health: 8, metro: 5, retail: 8, parks: 6, worship: 7 },
  { slug: 'saadiyat-island', nameEn: 'Saadiyat Island', nameAr: 'جزيرة سADIYAT', emirate: 'abu_dhabi', lat: 24.5560, lng: 54.4340, median: 2200, rent: 130000, yield: 5.8, score: 86, s30d: 2.1, s1y: 19.4, t30: 123, total: 10200, schools: 8, health: 7, metro: 3, retail: 7, parks: 9, worship: 6 },
...
async function main() {
  ...
  const communityRecords: Record<string, { id: string; median: number; rent: number }> = {}
  for (const c of communities) {
    const existing = await prisma.community.findUnique({ where: { slug: c.slug } })
    const rec = existing
      ? await prisma.community.update({ where: { slug: c.slug }, data: { medianAedSqft: c.median, medianAnnualRentAed: c.rent, grossYieldPct: c.yield, neighbourhoodScore: c.score, priceChange30d: c.s30d, priceChange1y: c.s1y, transactionCount30d: c.t30, totalTransactions: c.total, scoreSchools: c.schools, scoreHealthcare: c.health, scoreMetro: c.metro, scoreRetail: c.retail, scoreParks: c.parks, scoreWorship: c.worship } })
      : await prisma.community.create({ data: { slug: c.slug, nameEn: c.nameEn, nameAr: c.nameAr, emirate: c.emirate, latitude: c.lat, longitude: c.lng, medianAedSqft: c.median, medianAnnualRentAed: c.rent, grossYieldPct: c.yield, neighbourhoodScore: c.score, priceChange30d: c.s30d, priceChange1y: c.s1y, transactionCount30d: c.t30, totalTransactions: c.total, scoreSchools: c.schools, scoreHealthcare: c.health, scoreMetro: c.metro, scoreRetail: c.retail, scoreParks: c.parks, scoreWorship: c.worship } })
    communityRecords[c.slug] = { id: rec.id, median: c.median, rent: c.rent }
  }
  console.log(`Communities: ${communities.length}`)

  for (const c of communities) {
    const cid = communityRecords[c.slug].id
    const existingTx = await prisma.transaction.count({ where: { communityId: cid } })
    ...
    for (let i = 0; i < 15; i++) {
      ...
      txs.push({ dldId: `DLD-${c.slug.substring(0, 3).toUpperCase()}-${Date.now()}-${i}-${Math.random().toString(36).slice(2,6)}`, communityId: cid, transactionType: Math.random() > 0.15 ? 'sale' : 'off_plan_sale', propertyType: pt, beds, areaSqft: Math.round(area), priceAed: price, pricePerSqft: ppsf, transactionDate: txDate })
    }
  ...
  for (const c of communities) {
    const cid = communityRecords[c.slug].id
    const existingListings = await prisma.listing.count({ where: { communityId: cid } })
    if (existingListings > 0) continue
    const listings = []
    ...
    for (let i = 0; i < numListings; i++) {
      ...
      listings.push({ externalId: `${sources[i % 3]}-${c.slug}-${i}-${Math.random().toString(36).slice(2,6)}`, source: sources[i % 3], communityId: cid, purpose: isRent ? 'rent' : 'sale', propertyType: pt, beds, baths: Math.max(1, beds), areaSqft: Math.round(area), priceAed: price, pricePerSqft: isRent ? Math.round(price * 12 / area) : ppsf, furnished: ['furnished', 'unfurnished', 'partly'][Math.floor(Math.random() * 3)], completion: Math.random() > 0.8 ? 'off_plan' : 'ready', agentName: agents[Math.floor(Math.random() * agents.length)], agencyName: agencies[Math.floor(Math.random() * agencies.length)], trakheesiPermit: `TRK-${100000 + Math.floor(Math.random() * 900000)}`, latitude: c.lat + (Math.random() - 0.5) * 0.02, longitude: c.lng + (Math.random() - 0.5) * 0.02, listedAt: new Date(Date.now() - Math.floor(Math.random() * 90) * 86400000), isDeal: ppsf < c.median * 0.9, title: `${beds}BR ${pt.charAt(0).toUpperCase() + pt.slice(1)} in ${c.nameEn}` })
    }
  ...
  for (const slug of watchSlugs) {
    ...
    if (cid) {
      const existing = await prisma.watchlist.findUnique({ where: { userId_communityId: { userId: user.id, communityId: cid } } })
      if (!existing) await prisma.watchlist.create({ data: { userId: user.id, communityId: cid } })
    }
  }
  console.log('Watchlist seeded')

  const existingPortfolios = await prisma.portfolio.count({ where: { userId: user.id } })
  if (existingPortfolios === 0) {
    const portfolioItems = [
      ...
      { slug: 'downtown-dubai', title: 'Burj Khalifa View Apt 801', type: 'apartment', beds: 1, area: 780, price: 2100000, date: new Date('2023-06-20'), rent: 110000, sc: 18000, mortgage: 0, rate: 0, term: 0 },
    ...
    for (const p of portfolioItems) {
      ...
      if (cid) await prisma.portfolio.create({ data: { userId: user.id, communityId: cid, title: p.title, propertyType: p.type, beds: p.beds, areaSqft: p.area, purchasePrice: p.price, purchaseDate: p.date, currentValue: Math.round(p.price * (1 + Math.random() * 0.2)), annualRent: p.rent, serviceCharge: p.sc, mortgageBalance: p.mortgage, mortgageRate: p.rate, mortgageTerm: p.term } })
    }
    console.log('Portfolio seeded')
  }

  const existingAlerts = await prisma.alert.count({ where: { userId: user.id } })
  if (existingAlerts === 0) {
    const alertTypes = ['below_market', 'price_drop', 'new_listing', 'yield_target']
    for (let i = 0; i < 5; i++) {
      const slug = watchSlugs[i % watchSlugs.length]
      const cid = communityRecords[slug]?.id
      if (cid) await prisma.alert.create({ data: { userId: user.id, communityId: cid, alertType: alertTypes[i % alertTypes.length], thresholdPct: 10 + Math.random() * 15, yieldTargetPct: alertTypes[i % alertTypes.length] === 'yield_target' ? 6 + Math.random() * 2 : null, propertyType: propertyTypes[i % 3], beds: i + 1, notifyPush: true, notifyEmail: true, notifyWhatsapp: i > 2 } })
    }
